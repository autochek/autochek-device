...
export class AutochekBGMDevice extends GlucosemeterDeviceBase {
	...
	async general_connect_precallback() {
		...
		this.startNotification(UUID_SERVICE_GLUCOSE, UUID_CHARACTERISTIC_MEASUREMENT)
		...
		this.startNotification(UUID_SERVICE_GLUCOSE, UUID_CHARACTERISTIC_RACP)
			.subscribe(async (buffer) => {
		...
		this.startNotification(UUID_SERVICE_CUSTOM, UUID_CHARACTERISTIC_CUSTOM)
			.subscribe((buffer) => {
